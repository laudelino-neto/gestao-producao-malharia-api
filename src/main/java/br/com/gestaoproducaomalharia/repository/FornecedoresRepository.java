...
public interface FornecedoresRepository extends JpaRepository<Fornecedor, Integer>{
	...
	@Query(value = "SELECT f FROM Fornecedor f WHERE Upper(f.nomeFantasia) LIKE Upper(:nomeFantasia) ORDER BY f.nomeFantasia ", 
			countQuery = "SELECT Count(f) FROM Fornecedor f WHERE Upper(f.nomeFantasia) LIKE Upper(:nomeFantasia)")
	public Page<Fornecedor> listarPor(String nomeFantasia, Pageable paginacao);
	
	@Query(value = "SELECT f FROM Fornecedor f WHERE f.id = :id")
	...
	@Modifying
	@Query(value = "UPDATE Fornecedor f SET f.status = :status WHERE f.id = :id")
